...
> | **Jira integration** | `--jira` flag on `/speckit.taskstoissues` creates an Epic → Story hierarchy with Fibonacci story points, plain-language tickets written for QA and Product (What this is / Why now / How to test / Risk, plus customer, admin and Support notes when the spec has them), acceptance criteria and engineering notes in Jira's own fields when the project has them, and version-pinned deep-links to spec.md/plan.md on the Bitbucket branch; `--sync` refreshes the generated parts of existing tickets from a changed spec without touching hand-written sections | Tickets flow into our Jira boards with correct hierarchy, sizing, AC where QA already works, and a way to keep that AC current without losing the team's edits |
...
This will create tickets for each task and update the task files with actual ticket keys. Before creating anything, it checks the Jira project's fields: it proposes an issue type such as Tech Debt for internal work, finds the right story points field, and uses the Acceptance Criteria, Engineering Notes and Feature Flag fields when they exist. Each story is created as a demo-able vertical slice, written for QA and Product first: a title that names the specific thing being changed, product display names instead of code names, and **What this is / Why now / How to test / Risk** sections, plus **What customers will notice**, **What changes for admins** and **Note for Support and CSM** when the spec has something to say. These are drawn from the whole spec (Experience Vision, Non-Goals, Adoption & Rollout), not just the user story. The story's full acceptance criteria are copied word for word from spec.md into Jira's Acceptance Criteria field (or the description, if the project has no such field), so QA can work entirely inside the tracker. An **Engineering** section (or the Engineering Notes field) holds the task checklist, with done and dropped tasks marked, and a version-pinned deep-link back to spec.md.
